...
cli_parse_mbox(const char *dir, cli_ctx *ctx)
{
...
#endif
    //initialise 是一个thread？运行资源？真的该好好看看thread的内容了。
    ...
    if (strncmp(buffer, "From ", 5) == 0) {
        ...
        int messagenumber;
        //POST:只能使用message.c里的函数去更改里面的内容。
        ...
        bool heuristicFound = false;
        //POST: parse email file inject input file, buffer, ctx; output message* body 如果只有一个email就直接用parseEmailFile就可以！
...
parseEmailBody(message *messageIn, text *textIn, mbox_ctx *mctx, unsigned int recursion_level)
{
    ...
    const int doCheckUrls       = (engine->dboptions & CL_DB_PHISHING_URLS && (DCONF_PHISHING & PHISHING_CONF_ENGINE)) || engine->dboptions & CL_DB_TRACKING_URLS;//判断使用engine的conf why？后面插入值得深究！
#if HAVE_JSON
    ...
    cli_dbgmsg("in parseEmailBody, %u files saved so far\n",
               mctx->files);
    //POST: recursion limit; files limit; 
    ...
    if (mainMessage && (messageGetBody(mainMessage) != NULL)) {
        ...
        switch (mimeType) {
            ...
            case MULTIPART:
                ...
                //上面的code好像只是为了找到一个valid的boundary？？对，找到当前是不是在boundary处而已。
                //POST:t_line->t_next是去找下一个出现的位置？ line 和text 和message这三者是什么关系？？？
                
                if (t_line == NULL) {
                    //cli_errmsg("multipart t_line null\n");
                ...
                inhead     = 1;
                inMimeHead = 0;
                //开始进行preset，将multipart的各个部分解析出来保存成文件？
                /*
                ...
                for (multiparts = 0; t_line && !infected; multiparts++) {
                    ...
                    if (t_line == NULL) {
                        ...
                        continue;
                    }//空白multiparts就不存储，重新开始分配内存。
                    //接下来开始循环get line。
                    do {
                        const char *line = lineGetData(t_line->t_line);
                        //cli_errmsg("multiparts line: %s\n",line);
                        //POST: inject multiparts line!
                        /*
                        ...
                        */
                        //mime head如content-type和内容中间有空白行的间隔。
                        if (inMimeHead) { /* continuation line */
                            ...
                            //嘿，您猜怎么着，前面都是跳过header，设置mimetype等等，对于内容，直接放进里面
                ...
                //分隔结束了？？感觉没有存文件的操作啊？只是分离出各个multiparts还没开始对其进行操作。
                free((char *)boundary);
...
checkURLs(message *mainMessage, mbox_ctx *mctx, mbox_status *rc, int is_html)
{
    ...
    hrefs.tag = hrefs.value = NULL;
    hrefs.contents          = NULL;
    //setup hrefs, all the reference?
    b = getHrefs(mainMessage, &hrefs);
    
    // for(i;i<hrefs.count;i++){
    //     cli_errmsg("href tag %d tag: %s; content: %s; value: %s;\n",hrefs.count,hrefs.tag[i],hrefs.contents[i],hrefs.value[i]);
    // }
    ...
    if (b) {
        if (hrefs.scanContents) {
            ...
            cli_errmsg("phishing scan finish!\n");
            if(mctx->ctx->engine->dboptions & CL_DB_TRACKING_URLS){
                if(trackingScan(mctx->ctx,&hrefs) == CL_VIRUS){
                    mainMessage->isInfected = true;
                    *rc = VIRUS;
