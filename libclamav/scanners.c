...
static cl_error_t cli_scanmail(cli_ctx *ctx)
{
     ...
     */ //POST:scan mail 的同时，分离出attachment，放到tmp dir。
     //cli_mbox 是cli_parse_mbox的wrapper function
    ...
    //POST: ahhhhhh just realize that the dir is now have a html in it hahahha
    ret = cli_magic_scan_dir(dir, ctx);
...
static inline void perf_init(cli_ctx *ctx)
{
    //POST: 这是为了为了不报"unused parameter"的warnings！
    UNUSEDPARAM(ctx);
}
static inline void perf_start(cli_ctx *ctx, int id)
{
...
#endif
    //POST:omit the compressed file type;
    if ((typercg) &&
...
cl_error_t cli_magic_scan(cli_ctx *ctx, cli_file_t type)
{
    ...
    //POST: CL_CLEAN == CL_SUCCESS! HERE 判断读取的文件有没有超过maxfileNumber || maxDataBlocks
    ...
    //POST:推测，每次真实进行操作，都需要使用perf_start,perf_stop函数来开新的thread？或者说记录event？
    /*
...
#endif
    //POST: dispatch_prescan_callback here is the scan process start?
    //prescan 也可以检测file有无病毒？
    ...
    //POST:如果不允许检测任何模式，则只能以raw模式运行检测。
    ...
    //POST: inject message
    //cli_errmsg("scan message?\n");
    //POST:执行一些判断，不是最终判断依据，只是内部是否有错误的一个verdict
    ret = dispatch_prescan_callback(ctx->engine->cb_pre_scan, ctx, filetype);
    if (CL_CLEAN != ret) {
        if (ret == CL_VIRUS) {
            ret = cli_check_fp(ctx, NULL);
    ...
    //cli_errmsg("scan message 1 ?\n");
    //POST: first loop doesnot reach here!
    //cli_errmsg("scan messages?\n");
...
static cl_error_t scan_common(cl_fmap_t *map, const char *filepath, const char **virname, unsigned long int *scanned, const struct cl_engine *engine, struct cl_scan_options *scanoptions, void *context)
{
    ...
    ctx.virname = virname;
    ctx.scanned = scanned;
    ctx.options = malloc(sizeof(struct cl_scan_options)); //POST:重新分配内存。是因为可能修改？
    ...
    //POST:最底层的recursion_stack设置;
    //scancommon 是整个scan过程的最高层，现在也是在设置最高层的stack
    ...
    //POST: 啊，perf_init 在scan_common中开始，但是perf_start在scan_magic
    perf_init(&ctx);
    ...
    //POST: start first layer scan(with cl_type_any)
...
cl_error_t cl_scandesc_callback(int desc, const char *filename, const char **virname, unsigned long int *scanned, const struct cl_engine *engine, struct cl_scan_options *scanoptions, void *context)
{
    ...
    STATBUF sb;
    char *filename_base = NULL;
    //POST: for robustness, validate file descriptor status.
    if (FSTAT(desc, &sb) == -1) {
    ...
    if ((engine->maxfilesize > 0) && ((uint64_t)sb.st_size > engine->maxfilesize)) {
        ...
        if (scanoptions->heuristic & CL_SCAN_HEURISTIC_EXCEEDS_MAX) {
            //POST: if set heuristic, then file too large meanbs cb_virrus_found!
    ...
    //POST: get file name base by file name. but i wonder how can we send message here?
    if (NULL != filename) {
        (void)cli_basename(filename, strlen(filename), &filename_base);
    }
    //cli_errmsg("file name base: %s\n",filename_base);
    //POST: here fmap is a self developed model.
