...
static void scanfile(const char *filename, struct cl_engine *engine, const struct optstruct *opts, struct cl_scan_options *options)
{
    ...
    memset(&chain, 0, sizeof(chain));//initialize with 0;but what is metachain?
    if (optget(opts, "archive-verbose")->enabled) {
        chain.chains = malloc(sizeof(char **));//这个用法？？？
        if (chain.chains) {
    ...
    //POST:past in file descriptor, ret == CL_VIRUS virus detected.
    if ((ret = cl_scandesc_callback(fd, filename, &virname, &info.blocks, engine, options, &data)) == CL_VIRUS) {
        if (optget(opts, "archive-verbose")->enabled) {
            if (chain.nchains > 1) {
                ...
                //data struture is the data callback,
                logg("~%s!(%llu): %s FOUND\n", filename, (long long unsigned)(chain.lastvir - 1), virname);
            }
        }
        info.files++;
        info.ifiles++;
        //POST: info is global variable
        if (bell)
            fprintf(stderr, "\007");
    } else if (ret == CL_CLEAN) {
        //POST： detect no virus; OK
...
int scanmanager(const struct optstruct *opts)
{
    ...
    //POST:cl_engine_initialize  initialize dynamic config！
    ...
    //POST:here load the callback, but where does the cl_engine start the load process?
    ...
    //POST: here start the load database process?
    ...
    if (!opts->filename && !optget(opts, "file-list")->enabled) {
        ...
        //mprintf("injected in file process\n");
        //最后就是file-list形式传文件列表。
        if (opts->filename && optget(opts, "file-list")->enabled)
            logg("^Only scanning files from --file-list (files passed at cmdline are ignored)\n");
        //int ret,修改ret的内容。一个一个的取出file的内容？然后放到file[i]里？
