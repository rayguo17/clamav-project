...
int main(int argc, char **argv)
{
...
#ifndef _WIN32  //if not def (_WIN32) ps: if in win_32 machine, _WIN32 will be 1; if it is not win32 machines, then do.
    struct passwd *user = NULL;
    struct sigaction sa; //signal helper
    int dropPrivRet = 0;
#endif
#if defined(C_LINUX) || (defined(RLIMIT_DATA) && defined(C_BSD))
    struct rlimit rlim;
#endif
    time_t currtime; //defineing some things could be configure in .config file
    ...
    sigaction(SIGHUP, &sa, NULL); //注册信号，忽略SIGHUP和SIGUSR2，old action指针为NULL
    sigaction(SIGUSR2, &sa, NULL);
    if (!setlocale(LC_CTYPE, "")) { //调用系统api，设置locale（局限于当前环境）
    ...
    //如果opts help则输出help然后退出（只输出help）（根据需要的功能查找，拓展性高）
    ...
    if (optget(opts, "debug")->enabled) {
        ...
        rlim.rlim_cur = rlim.rlim_max = RLIM_INFINITY; //预定义参数，表示拿尽量多的资源
        if (setrlimit(RLIMIT_CORE, &rlim) < 0) //要debug的话就不能限制内核转存文件的最大长度。
            perror("setrlimit");
#endif
        debug_mode = 1;
    }
    ...
    //查找完成后在这里运行（是不是有编译器优化的理念，不要在for里面写代码。）得到条件后进行执行。
    if (j < argc) {
    ...
    //daemon 程序之间的依赖 这里是从systemd拿到处于listen状态的socket，用于服务端唤醒（来自客户端）
    num_fd = sd_listen_fds(0);

    /* parse the config file */
    cfgfile = optget(opts, "config-file")->strarg; //从config-file 表项拿到config文件的位置
    pt      = strdup(cfgfile);//复制字符串到堆中。（新开辟的空间）
    if (pt == NULL) {
        fprintf(stderr, "ERROR: Unable to allocate memory for config file\n");
        return 1;
    }
    if ((opts = optparse(cfgfile, 0, NULL, 1, OPT_CLAMD, 0, opts)) == NULL) {//尝试分析cfgfile(put config in opts).默认配置可能在optparse已经设置好
        fprintf(stderr, "ERROR: Can't open/parse the config file %s\n", pt);
        free(pt);
        return 1;
    }
    free(pt);//pt没有使用就free掉了？？
    //看有没有user选项
...
#endif //TODO:
        gengine = engine;
        atexit(free_engine);//when exit free engine(cause valgrind mistakenly print error)
        daemonizeRet = daemonize_parent_wait(user_name, logg_file);//wait for child process, but no child process have been created!? or wait for SIGINT(interrupt ctrl+c )
        if (daemonizeRet < 0) {
    ...
    /* save the PID */ 
    mainpid = getpid();//get the client process? //save main thread process id
    if ((opt = optget(opts, "PidFile"))->enabled) {//put it in the clamd.pid file (check config file)
        FILE *fd;
    ...
    do { /* logger initialized */
        ...
        /* check socket type */
        //three types of functional tcp socket / local socket / file descriptor from client / could run multiple socket at the sametime?
